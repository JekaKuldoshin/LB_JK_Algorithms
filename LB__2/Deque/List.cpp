...
bool deque::check() {        //\\//\\ Функ-я проверки на наличие элементов очереди //\\//\\ 
	if (left == NULL) {        //Если пуст стек, то выводим...
		system("cls");
...
void deque::create_front(int num) {       //\\//\\ Функ-я для создания списка //\\//\\ 
	int a = 1;     //Счеткик элементов
	char d;     //Переменная для ввода
	for (int i = 0; i < num; i++) {
		cout << "Введите данные для " << a++ << " позиции элемента стека -> ";
		cin >> d;
		push_front(d);    //Переход в функ-ю добавления элементов в конец списка
	}
	a = 0;
	system("cls");
}
